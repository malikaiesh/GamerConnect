...
function RoomCard({ room }: { room: ExploreRoom }) {
...
export default function ExploreRoomsPage() {
  const { data: rooms, isLoading, error } = useQuery({
    queryKey: ["/api/rooms", { category: "explore" }],
  });
  ...
  return (
    <div className="min-h-screen flex flex-col bg-background">
      ...
      <section className="relative py-16 overflow-hidden">
        ...
        <div className="absolute inset-0 bg-gradient-to-br from-blue-500/20 via-background to-background/90"></div>
        {/* Decorative elements */}
        ...
        <div className="container relative mx-auto px-4">
          <div className="max-w-2xl mx-auto text-center">
            <div className="inline-block mb-4 px-4 py-1 border border-blue-500/30 rounded-full bg-blue-500/10 text-blue-600 text-sm font-medium tracking-wide">
              EXPLORE ROOMS
            </div>
            <h1 className="heading-xl mb-6 text-transparent bg-clip-text bg-gradient-to-r from-blue-500 to-blue-600 font-extrabold">
              🌍 Explore Rooms
            </h1>
            <p className="text-lg md:text-xl font-medium text-foreground/90 leading-relaxed">
              Discover new rooms and communities to connect with people around the world
            </p>
          </div>
        </div>
      </section>
        ...
        <div className="container mx-auto px-4">
          {isLoading ? (
            ...
            <div className="text-center py-12">
              <p className="text-red-500">Failed to load explore rooms. Please try again.</p>
            </div>
          ) : !rooms?.length ? (
            <div className="text-center py-12">
              <div className="p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg inline-block">
                ...
                <p className="text-gray-600 dark:text-gray-300">No rooms to explore right now.</p>
                <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Check back later for new communities!</p>
              </div>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
              {rooms?.map((room: ExploreRoom) => (
