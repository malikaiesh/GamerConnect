...
export function registerSeoSchemaRoutes(app: express.Express) {
  ...
  app.post("/api/public/seo-schemas/bulk-generate", async (req, res) => {
    try {
      ...
      if (contentType === 'game') {
        const gamesResult = await storage.getGames({ page: 1, limit: 10 });
        const games = gamesResult.games;
        for (const game of games.slice(0, 10)) { // Limit to 10 for demo
          try {
            const schemaData = await generator.generateGameSchema(game);
            if (schemaData) {
              const schemaToCreate = {
                schemaType: 'VideoGame' as const,
                contentType: 'game' as const,
                contentId: game.id,
                name: `Auto: ${game.title} Game Schema`,
              ...
              const created = await storage.createSeoSchema(schemaToCreate);
